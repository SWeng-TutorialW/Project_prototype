...
public class RegistrationController {
    ...
    private ChoiceBox<String> selectStoreCB; // Value injected by FXMLLoader

    @FXML // fx:id="selectStoreLbl"
    private Label selectStoreLbl; // Value injected by FXMLLoader

    private int accType = SimpleClient.getSelectedAccType();
    ...
    public void sendSubscriptionDetails(String id, String credit) {
        ...
        try {
            SimpleClient.getClient().sendToServer(
                    new LoginRegCheck(regUser, regPass, email, 0, accType, id, credit)
            );
    ...
    void RegToSys(MouseEvent event) {
        ...
        if ((Objects.equals(confPass, regPass) || Objects.equals(confPass2, regPass2))
                ...
                email.length() >= 4) && selectedStore != null) {
            try {
                if (accType == 0) {
                    // String store = selectStoreCB.getValue();
                    regErrMsgLbl.setVisible(false);
                    SimpleClient.getClient().sendToServer(new LoginRegCheck(regUser, regPass, email, 0, accType, selectedStore));
                } else if (accType == 1) {
                    regErrMsgLbl.setVisible(false);
                    SimpleClient.getClient().sendToServer(new LoginRegCheck(regUser, regPass, email, 0, accType));
                } else if (accType == 2) {
    ...
    void initialize() {
        ...
        if (accType == 0) {
            selectStoreCB.getItems().addAll("Haifa", "Tel Aviv", "Beer Sheva");
        }
