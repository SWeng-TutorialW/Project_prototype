...
public class DatabaseInitializer {
    ...
    private static void initializeOrders(Session session) {
        ...
        int ordersPerUser = totalOrders / usersCount;
        int extraOrders = totalOrders % usersCount;
        ...
        for (int u = 0; u < usersCount; u++) {
            ...
            int userOrders = ordersPerUser + (u < extraOrders ? 1 : 0);
            for (int i = 0; i < userOrders; i++, orderIdx++) {
                ...
                if (delivery) {
                    order.setCity("SampleCity");
                    order.setStreetAddress("Sample Street " + (rand.nextInt(100) + 1));
                ...
                // Add some day offset for more variety
                ...
                if (deliveryTime.after(nowDate)) {
                    // Order date is a random time before today (not in the future)
                    long maxOrderTime = nowDate.getTime() - 3600_000L; // at least 1 hour before now
                    long minOrderTime = maxOrderTime - (14L * 24 * 3600_000L); // up to 14 days ago
                    ...
                    // Set order date to be a little before delivery/pickup time (between 1 and 48 hours before)
                ...
                if (status.equals("CANCELLED")) {
                    // Decide how long before delivery/pickup the order was cancelled
                    int hoursBefore = rand.nextInt(5); // 0-4 hours before
                    long cancelTime = deliveryTime.getTime() - (hoursBefore * 3600_000L);
                    order.setCancellationDate(new java.util.Date(cancelTime));
                    if (hoursBefore >= 3) {
                        order.setRefundAmount(order.getTotalAmount()); // 100% refund
                    } else if (hoursBefore >= 1) {
                        order.setRefundAmount(order.getTotalAmount() * 0.5); // 50% refund
                    } else {
                        order.setRefundAmount(0.0); // 0% refund
                    }
